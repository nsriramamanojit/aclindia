module AutoCompleteMacrosHelper      
  ...
  def auto_complete_result(entries, field,query)
    return unless entries
    if entries.blank?
      items =  content_tag("li","No Results Found for <font color='yellow'>#{query}</font>")
      content_tag("ul", items)
    else
      items = entries.map { |entry| content_tag("li", highlight(truncate(entry[field], :length => 60), query))}
